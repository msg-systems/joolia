// originally from https://github.com/chrum/ngx-autosize

import { ElementRef, HostListener, Directive, Input, NgZone, OnDestroy, OnChanges, AfterContentChecked } from '@angular/core';
    ...
    _onTextAreaFound() {
        this._zone.runOutsideAngular(() => {
            fromEvent(window, 'resize')
                .pipe(debounceTime(200), distinctUntilChanged(), takeUntil(this._destroyed$))
                .subscribe(() => {
                    this._zone.run(() => {
                        this.adjust();
                    });
