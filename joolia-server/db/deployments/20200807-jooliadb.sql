...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`%` SQL SECURITY DEFINER */
/*!50001 VIEW `workspace_view` AS select `w`.`id` AS `id`,`w`.`name` AS `name`,`w`.`description` AS `description`,`w`.`licensesCount` AS `licensesCount`,(select count(`f`.`id`) from `format` `f` where (`f`.`workspaceId` = `w`.`id`)) AS `formatCount`,(select count(`m`.`userId`) from `workspace_members_user` `m` where (`m`.`workspaceId` = `w`.`id`)) AS `memberCount` from `workspace` `w` */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
