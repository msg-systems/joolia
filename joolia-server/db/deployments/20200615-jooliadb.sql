...
/*!50001 VIEW `format_view` AS select `t`.`formatId` AS `id`,`t`.`memberCount` AS `_memberCount`,`t`.`teamCount` AS `_teamCount`,`t`.`submissionCount` AS `_submissionCount`,`t`.`phaseCount` AS `_phaseCount`,`t`.`activityCount` AS `_activityCount`,`t`.`commentCount` AS `_commentCount`,min(`t`.`phaseStartDate`) AS `startDate`,max(`t`.`phaseEndDate`) AS `endDate` from (select `f`.`id` AS `formatId`,`p`.`startDate` AS `phaseStartDate`,(select count(1) from `jooliadb`.`format_member` where (`jooliadb`.`format_member`.`formatId` = `f`.`id`)) AS `memberCount`,(select count(1) from `jooliadb`.`team` where (`jooliadb`.`team`.`formatId` = `f`.`id`)) AS `teamCount`,(select count(1) from ((`jooliadb`.`submission` `s` join `jooliadb`.`activity` `a` on((`s`.`activityId` = `a`.`id`))) join `jooliadb`.`phase` `p` on((`a`.`phaseId` = `p`.`id`))) where (`p`.`formatId` = `f`.`id`)) AS `submissionCount`,(select count(1) from `jooliadb`.`phase` where (`jooliadb`.`phase`.`formatId` = `f`.`id`)) AS `phaseCount`,(select count(1) from (`jooliadb`.`activity` `a` join `jooliadb`.`phase` `p` on((`a`.`phaseId` = `p`.`id`))) where (`p`.`formatId` = `f`.`id`)) AS `activityCount`,(select count(1) from (((`jooliadb`.`feedback` `fb` join `jooliadb`.`submission` `s` on((`fb`.`submissionId` = `s`.`id`))) join `jooliadb`.`activity` `a` on((`s`.`activityId` = `a`.`id`))) join `jooliadb`.`phase` `p` on((`a`.`phaseId` = `p`.`id`))) where (`p`.`formatId` = `f`.`id`)) AS `commentCount`,sum(`a`.`duration`) AS `phaseDuration`,(`p`.`startDate` + interval sum(`a`.`duration`) minute) AS `phaseEndDate` from ((`jooliadb`.`phase` `p` join `jooliadb`.`format` `f` on((`p`.`formatId` = `f`.`id`))) join `jooliadb`.`activity` `a` on((`a`.`phaseId` = `p`.`id`))) group by `f`.`id`,`f`.`name`,`p`.`startDate`) `t` group by `t`.`formatId` */;
